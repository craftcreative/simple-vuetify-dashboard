...
export default {
  ...
  methods:{
    ...
    metricsData(ssid, sessionEnd, sessionStart){
      axios.get('http://localhost:4000/api/syncc/metric/'+ssid+'/'+sessionEnd).then(response => {
    ...
    addSession(username) {
      axios.get('http://localhost:4000/api/syncc/sessions/'+username)
      .then(response => {
  ...
  created() {
    ...
    this.synccUser = this.$store.getters.synccUser.display_name
    this.addSession(this.$store.getters.synccUser.user_name)
  }
}
</script>
